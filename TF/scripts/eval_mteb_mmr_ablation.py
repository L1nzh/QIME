...
def parse_args():
    ...
    parser.add_argument("--topks", nargs="+", type=int, default=[256])
...
def save_dim_meta(model, task_name):
    path = "../results_ab/{model_name}/no_model_name_available/no_revision_available/indices.json".format(model_name=model.model_name)
...
def run_topk_worker(topk_vals, task_list, gpu_id):
    device = choose_device(gpu_id)
    for topk_val in topk_vals:
        model = EncoderModel(topk=topk_val, device=device)
        for task_name in tqdm(task_list, desc="topk {0}".format(topk_val)):
            model.one_indices = []
            evaluator = mteb.MTEB(tasks=[task_name])
            evaluator.run(model, output_folder="../results_ab/{model_name}".format(model_name=model.model_name))
            save_dim_meta(model, task_name)
...
def run_binary(topks, task_list, gpus):
    ...
    for gpu_id, subset in tqdm(splits, desc="launch topk workers"):
        if not subset:
            continue
        process = ctx.Process(target=run_topk_worker, args=(subset, task_list, gpu_id))
        process.start()
...
def run_hf(evaluator, model_name):
    ...
    evaluator.run(model, output_folder="../results_ab/")

def resolve_model_name(args):
    if args.model_kind == "binary":
        return "top{0}_binary_mmr".format(args.topks)
...
def main():
    ...
    if args.model_kind == "binary":
        run_binary(args.topks, task_names, args.gpus)
    else:
