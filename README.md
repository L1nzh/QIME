...
*   **Training-Free (TF) Variant**: Produces interpretable embeddings without any classifier training by directly leveraging cosine similarity between document and question embeddings.
...
├── TF/                # Training-Free variant
│   ├── code/          # Training-free encoding models
│   │   ├── test_model.py          # Top-k binary embedding model
│   │   ├── mmr_topk_model.py      # MMR-diversified binary embedding model
│   │   └── test_model_cos_mmr.py  # Cosine-valued sparse embedding model
│   ├── scripts/       # MTEB evaluation scripts for TF models
│   │   ├── eval_mteb.py           # Evaluate top-k binary model
│   │   ├── eval_mteb_mmr.py       # Evaluate MMR model
│   │   ├── eval_mteb_ablation.py  # Evaluate top-k binary with an alternate question set
│   │   ├── eval_mteb_mmr_ablation.py # MMR ablation runner
│   │   └── eval_mteb_mmr_lambda.py   # MMR lambda sweep helper
│   ├── dataset/       # Medical MTEB task list
│   └── data/          # Question sets generated by different LLMs
...
## Training-Free (TF) Variant

The `TF/` directory contains a **training-free** alternative to the classifier-based IMEC approach. Instead of training per-question MLP classifier heads, the TF variant directly uses a pretrained sentence encoder (`abhinand/MedEmbed-large-v0.1`) to compute cosine similarity between document embeddings and question embeddings, then selects the top-k most relevant questions to construct interpretable sparse embeddings.

### TF Model Variants

Three encoding strategies are provided in `TF/code/`:

1.  **Top-k Binary** (`test_model.py`): Selects the top-k questions with highest cosine similarity to the document and outputs a binary vector (1 at selected positions, 0 elsewhere).
2.  **MMR Binary** (`mmr_topk_model.py`): Uses Maximal Marginal Relevance (MMR) to select top-k questions, balancing relevance to the document with diversity among selected questions. Controlled by `mmr_diversity` (default 0.7).
3.  **Cosine-valued Sparse** (`test_model_cos_mmr.py`): Same as top-k selection but places the actual cosine similarity values at the selected positions instead of binary 1s.

### TF Usage

```python
import sys
sys.path.append("TF/code")
from mmr_topk_model import EncoderModel

# Initialize with top-k and MMR diversity parameters
model = EncoderModel(topk=128, mmr_diversity=0.7, que_path="TF/data/questions.json")

# Encode documents — no training needed
docs = ["Patient presents with severe chest pain.", "Treatment involves daily insulin injections."]
embeddings = model.encode(docs)

# embeddings shape: [2, num_questions] — sparse interpretable vectors
```

### TF Evaluation

Evaluation scripts in `TF/scripts/` run the TF models on medical MTEB benchmarks (Retrieval, STS, Clustering). They support configurable `--topks`, `--questions-path`, and multi-GPU parallelism.

```bash
# Evaluate top-k binary model
cd TF/scripts
python eval_mteb.py --topks 128 256 --questions-path ../data/questions.json

# Evaluate MMR model
python eval_mteb_mmr.py --topks 128 256 --questions-path ../data/questions.json
```

The task list is defined in `TF/dataset/datasets.yaml`. It currently includes the biomedical clustering, STS, and retrieval tasks used by the TF evaluation scripts.

Additional experiment helpers are also provided:

```bash
# Evaluate the top-k binary model with the non-medically grounded question set
python eval_mteb_ablation.py --topks 128 256 --questions-path ../data/linear_questions.json

# Run the MMR ablation helper
python eval_mteb_mmr_ablation.py --topks 256

# Sweep MMR lambda values at a fixed top-k
python eval_mteb_mmr_lambda.py --topk 256 --lambdas 0.1 0.3 0.5 0.7 0.9
```

### TF Question Sets

The `TF/data/` directory contains question sets generated by different LLMs:
*   `questions.json` — Default question set
*   `linear_questions.json` — Alternate question set without medical grounding, used by `eval_mteb_ablation.py` by default
*   `questions_gpt_5_mini.json` — Questions generated by GPT-5-mini
*   `questions_llama.json` — Questions generated by LLaMA
